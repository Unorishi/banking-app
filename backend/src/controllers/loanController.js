...
export const applyForLoan = async (req, res) => {
    try {
        ...
        res.status(400).json({ error: "Error applying for loan: " + err.message });
    }
};
...
export const getMyLoans = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error fetching loans: " + error.message });
  }
};
...
export const getLoanDetails = async (req, res) => {
    try {
        ...
        if(!loan){
            return res.status(404).json({ error: "Loan not found or u dont have access" });
        }
        res.json(loan);
    } catch (err) {
...
export const updateLoanStatus = async (req, res) => {
  ...
  try {
    ...
    await session.abortTransaction();
    session.endSession();
    res.status(500).json({ error: "Error updating loan status: " + error.message });
  }
};
...
export const getRepaymentSchedule = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error fetching repayment schedule: " + error.message });
  }
};
